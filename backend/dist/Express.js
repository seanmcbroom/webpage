...
app.use((req, res, next) => {
    ...
    // Add CSP headers
    res.setHeader("Content-Security-Policy", "default-src 'self' 'unsafe-inline' kirakirafoxx.com; img-src 'self' https: data:; font-src 'self' data: fonts.googleapis.com fonts.gstatic.com;");
    next();
});
...
app.listen(port, () => log(`[Express] Listening on port ${port} [http://localhost:${port}]`, {
    textColor: "blue",
}));
